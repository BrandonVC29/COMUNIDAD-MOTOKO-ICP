...
const SocialItem = (props) => {
    ...
    const handleUpdate = async (event) => {
        ...
        try {
            ...
            setLoading("Lo siento, hubo un error.");
            console.log(e);
        } finally {
            setVisible(false);
        }
    ...
    return(
        <div className="border border-gray-500 p-2">
            <p className="border-b border-gray-500"><strong>Subido por: </strong>{post[1].creator}</p>
            <div className="mb-2">
                <p>{post[1].message}</p>
            ...
            <div className={`${visible ? `flex` : `hidden`} flex-col items-center justify center w-full space-y-2 my-2`}>
                ...
                <button className="w-full bg-gray-950 hover:bg-gray-900 text-white p-2 font-bold" onClick={handleUpdate}>Actualizar</button>
            </div>
            <p>{loading}</p>
            <div className={`${visible ? `hidden` : `flex`} items-center justify center w-full space-x-2 mt-2"`}>
                <button className="w-full bg-gray-950 hover:bg-gray-900 text-white p-2 font-bold" onClick={() => setVisible(true)}>Editar</button>
                <button className="w-full bg-gray-950 hover:bg-gray-900 text-white p-2 font-bold" onClick={handleDelete}>Borrar</button>
            </div>
        </div>
    );
}
